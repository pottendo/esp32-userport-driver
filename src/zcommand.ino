...
void ZCommand::parseConfigOptions(String configArguments[])
{
  ...
  if ((str != null) && (str.length() > 0))
  {
    ...
    for (int i = 0; i < str.length(); i++)
    {
      ...
      {
        if (v2 &&
            ((argn == CFG_WIFISSI) || (argn == CFG_WIFIPW) || (argn == CFG_TIMEZONE) || (argn == CFG_TIMEFMT) || (argn == CFG_TIMEURL) || (argn == CFG_PRINTSPEC) || (argn == CFG_HOSTNAME) || (argn == CFG_TERMTYPE)))
        {
          char hex[256];
...
ZResult ZCommand::doConnectCommand(int vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers)
{
  ...
  {
    if ((WiFi.status() != WL_CONNECTED) && (vval == 1) && (conns == null))
    {
      if (wifiSSI.length() == 0)
        return ZERROR;
      debugPrintf("Connecting to %s\n", wifiSSI.c_str());
...
ZResult ZCommand::doSerialCommand()
{
  ...
  //debugPrintf("%s: sbuf = %s\n", __FUNCTION__, sbuf.c_str());
...
#ifdef INCLUDE_SD_SHELL
  serial.prints("ZiModem/PAR WiFi Firmware v");
#else
    ...
    //&& (nextConn->isConnected())) // being connected is not required to have buffered bytes waiting!
    {
      ...
      if (maxBytes > 0)
      {
        if ((nextConn->delimiters[0] != 0) || (delimiters[0] != 0))
        {
          ...
          if ((lastLen >= packetSize) || ((lastLen > 0) && ((strchr(nextConn->delimiters, lastBuf[lastLen - 1]) != null) || (strchr(delimiters, lastBuf[lastLen - 1]) != null))))
            lastLen = 0;
          int bytesRemain = maxBytes;
          while ((bytesRemain > 0) && (lastLen < packetSize) && ((lastLen == 0) || ((strchr(nextConn->delimiters, lastBuf[lastLen - 1]) == null) && (strchr(delimiters, lastBuf[lastLen - 1]) == null))))
          {
